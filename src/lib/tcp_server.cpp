...
tcp_server::tcp_server(unsigned short port, asio::io_context &context)
    ...
    , server{[this](asio::ip::tcp::socket &&new_user) {
                 ...
                 users.emplace(max_id, new client_connection{
...
void tcp_server::success_write_handler(size_t id) {
    debug_fprintf(stdout, "Successfully written (id : %5zu) : %5zu packages\n", id, ++counter[id].first);
}

void tcp_server::error_write_handler(size_t id, std::string_view error_message) {
...
void tcp_server::success_read_handler(size_t id, std::variant<message, sign_in, sign_up, special_signal> &&package) {
...
void tcp_server::error_read_handler(size_t id, std::string_view error_message) {
